...
import CertificateAdmin from './CertificateAdmin/CertificateAdmin'
import { useLoaderData } from 'react-router-dom'
import { useEffect } from 'react'

const AllCertificateAdmin = () => {
        ...
        const { data: allCertificate = [], refetch } = useQuery({
        queryKey: ['all-certificate'],
        queryFn: async () => {
            const { data } = await axiosSecure.get('academic-docs-get-admin?text=certificates')
     ...
     // Pagination
        ...
        useEffect(() => {
            const fetchDataPagination = async () => {
                const { data } = await axiosSecure.get(`/data-pagination?page=${currentPage}&size=${itemPerPage}&type=certificatesData`)
                setCertificates(data)
            }
            fetchDataPagination()
    ...
    const handleSearch = async val => {
        const { data } = await axiosSecure.get(`/certificate-data-search?search=${val}`)
        setCertificates(data)
    }
  return (
    <div className='max-w-6xl mx-auto'>
     <p className='text-green-500 text-3xl font-bold py-5'>All Certificate Information</p>
      <div className='text-center'>
        ...
        <div className='grid lg:grid-cols-2 gap-4'>
                {
                    certificates.map(certificate => <CertificateAdmin
...
export default AllCertificateAdmin
